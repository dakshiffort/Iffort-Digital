# Landing Page v2 — "Book a Meeting" (Ad Campaign)
...
- **Vercel**: create a second project pointing at this repo with **Root Directory =
  `landing-v2`** (no framework, no build command). Keeps the main site deployment untouched.
- **Netlify / Cloudflare Pages / GitHub Pages**: publish the `landing-v2` folder as the
  site root.

If deployed on the same Vercel project as the main site, the page is reachable at
`/landing-v2/` and the lead form posts to the existing `/api/send-email` function. If
